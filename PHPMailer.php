...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['actionLogout'])) {
        ...
        if ($_POST['actionEmail'] == 'Send Email') {//Send Email button was pressed
            ...
            $sendToArray = $mySqlFunctions->getArrayOfRecipientsInMailBox($_COOKIE['emailId'], 'to');
            ...
            $addCCArray = $mySqlFunctions->getArrayOfRecipientsInMailBox($_COOKIE['emailId'], 'cc');

            $addBCCArray = $mySqlFunctions->getArrayOfRecipientsInMailBox($_COOKIE['emailId'], 'bcc');
            ...
            //echo $esito;
            if ($esito) {
                $result = $mySqlFunctions->registerEmailInviata($_COOKIE['emailId']);
                //echo $result;exit();
                if ($result) {
