...
//  XAMPP
//
//  Created by Roberto Della Grotta on 2014-10-09.
//  Copyright 2014 Roberto Della Grotta. All rights reserved.
//


/*
TODO salvataggio email inviate dal server in apposita tabella
*/

/**
 * Registra evento relativo ad accesso fallito! nella tabella utenti_logger
 * @param unknown_type $msgType
 * @param unknown_type $msgBody
 * @param unknown_type $id_utente
...
function registerLogEventFailure($msgType, $msgBody, $id_utente, $user_email,$password, $ip) {
	if($id_utente == null){
		...
		" VALUES (NULL,'%s','%s',now(),0,null, '%s', '%s', '%s')",
		$msgBody, $msgType,
		mysql_real_escape_string($user_email),
		mysql_real_escape_string($password), $ip);
	}else{
...
function registerLogEvent($msgType, $msgBody, $id_utente, $ip) {
	if($id_utente != null){
		...
		" VALUES (%s,'%s','%s',now(),0,null, '%s')",
		$id_utente , $msgBody, $msgType, $ip);
	}else{
		$query = sprintf("INSERT INTO scuola.utenti_logger ".
		"(id_utente, message, msg_type, when_registered, msg_to_utente, msg_sent_time, ip_client)".
		" VALUES (NULL,'%s','%s',now(),0,null, '%s')",
		$msgBody, $msgType, $ip);
	}
 ...
 * @param unknown_type $id_request
 * @param unknown_type $hash
 * @param unknown_type $cognome
...
function existRequestPendingFor($id_request, $hash, $cognome, $nome, $email) {
	if(connectToMySqlWithParams('localhost:3307','root','myzconun')){
		...
		$id_request, $hash, $email, strtoupper($cognome), strtoupper($nome));
...
function setRequestConfirmedFor($hash, $id_request) {
	if(connectToMySqlWithParams('localhost:3307','root','myzconun')){
		...
		$query = sprintf("UPDATE scuola.change_password_request".
		 " SET confirmed = 1 WHERE id_request = %s AND hash = '%s'",
		$id_request, $hash);
...
function postChangePasswordRequest($cognome, $nome, $email, $hash) {
	...
	if(connectToMySqlWithParams('localhost:3307','root','myzconun')){
		...
		$query = sprintf("SELECT id_utente FROM scuola.utenti_scuola".
		 " WHERE cognome = upper('%s') AND nome = upper('%s') AND email = '%s'",
		$cognome, $nome, $email);
 ...
 * Un'altro caso in cui viene utilizzata è quando l'utente,
...
function setUserHasToChangePassword($user, $value) {
	if(connectToMySql()){
		...
		SET has_to_change_password = %s WHERE id_utente = %s",
		$value, $user);
 ...
 * questo si verifica quando viene richiesta una nuova password perchè persa
 * o dimenticata, allora l'utente che entra per la prima volta con le nuove
 * credenziali, dopo la presente verifica riceverà un invito ossessivo,
...
function retrievePassword($id_utente) {
	...
	$query = sprintf("SELECT password FROM scuola.utenti_scuola WHERE id_utente = %s",
	$id_utente);
 ...
 * è stato registrato nella tabella email_recipients. Questo id serve per
 * l'operazione di cancellazione del destinatario ultimo inserito.
 * @param unknown_type $id_email
 * @param unknown_type $param
 */
function getMaxIdRecipient($id_email,$param) {
	if ($param=='to') {
		if(connectToMySql()){
			$query = sprintf("SELECT max(id_recipient) as max_id_recipient FROM scuola.email_recipients
			WHERE id_email = %s  
			AND to_recipient = '1'",
			$id_email);
 ...
 * @param unknown_type $id_email
 * @param unknown_type $recip_email
 * @param unknown_type $recip_name
...
function addRecipientToEmail($id_email, $recip_email, $recip_name, $to, $cc, $bcc) {
	if(connectToMySql()){
		...
		"VALUES (%s,'%s','%s',%s,%s,%s)",
		$id_email, $recip_email, $recip_name, $to, $cc, $bcc);
...
function recipientExists($id_email, $recip_email, $recip_name) {
	if(connectToMySql()){
		...
		email = '%s' AND
		name = '%s'",
		$id_email, $recip_email, $recip_name);
...
function getMySqlServerParams($serverGroup) {
	if(connectToMySqlWithParams('localhost:3307','root','myzconun')){
		$query = sprintf("SELECT * FROM scuola.mysql_server_params WHERE  id_server_group = %s ",
		$serverGroup);
...
function createEmail($from_email, $from_name, $id_user) {
	...
	if(connectToMySql()){
		$query = sprintf("INSERT INTO scuola.email_store(from_email, from_name, id_user, subject, body) VALUES ('%s','%s', '%s', '<NO SUBJECT>', '<NO BODY>')",
		$from_email, $from_name,$id_user);
...
function getArrayOfRecipientsInMailBox($emailId, $mailbox) {
	if(connectToMySql()){
		if($mailbox == 'to'){
			$query = sprintf("SELECT * FROM scuola.email_recipients WHERE id_email = %s AND to_recipient = 1",
			$emailId);
		}elseif ($mailbox == 'cc') {
			$query = sprintf("SELECT * FROM scuola.email_recipients WHERE id_email = %s AND cc_recipient = 1",
			$emailId);
		}elseif ($mailbox == 'bcc') {
			$query = sprintf("SELECT * FROM scuola.email_recipients WHERE id_email = %s AND bcc_recipient = 1",
			$emailId);
		}
...
function saveEmailDraft($emailId, $subject, $body) {
	if(connectToMySql()){
		...
		SET subject = \"%s\", body = \"%s\" WHERE id_email = %s",
...
function getEmailSubject($emailId) {
	if(connectToMySql()){
		$query = sprintf("SELECT subject FROM scuola.email_store WHERE id_email = %s",
		$emailId);
...
function setEmailSubject($emailId, $subject) {
	if(connectToMySql()){
		$query = sprintf("UPDATE scuola.email_store
		SET subject = \"%s\" WHERE id_email = %s",
		mysql_real_escape_string($subject),
		($emailId));
...
function getEmailBody($emailId) {
	if(connectToMySql()){
		$query = sprintf("SELECT body FROM scuola.email_store WHERE id_email = %s",
		$emailId);
...
function userIsAdministrator($id_utente) {
	...
	$query = sprintf("SELECT user_is_admin FROM scuola.utenti_scuola WHERE id_utente = %s",
	$id_utente);
...
function removePendingFromUserRequest($user, $admin) {
	...
	" id_admin = %s, request_done_date = now() WHERE from_user = %s AND pending = 1",
	$admin, $user);
...
function registerEmailInviata($emailId) {
	...
	if(connectToMySql()){
		$query = sprintf("UPDATE scuola.email_store
			 SET email_inviata = 1 WHERE id_email = %s ",
		$emailId);
...
function getUserId($user_email, $password) {
	if(connectToMySql()){
		$query = sprintf("SELECT id_utente FROM scuola.utenti_scuola WHERE email = '%s'
		 AND password = '%s'",
		mysql_real_escape_string($user_email),
	mysql_real_escape_string(base64_encode(base64_encode($password))));
...
function getUserEmail($id_utente) {
	...
	$query = sprintf("SELECT email FROM scuola.utenti_scuola WHERE id_utente = %s",
	$id_utente);
...
function getUserName($id_utente) {
	...
	$query = sprintf("SELECT cognome, nome FROM scuola.utenti_scuola WHERE id_utente = %s",
	$id_utente);
...
function userHasRole($id_utente, $id_ruolo) {
	if(connectToMySql()){
		...
		WHERE id_utente= '%s' and id_ruolo = '%s'",
		mysql_real_escape_string($id_utente, $GLOBALS['link']),
		mysql_real_escape_string($id_ruolo, $GLOBALS['link']));
...
/**
 *
 * Lista degli utenti che hanno richiesto il cambio di password
 * @param unknown_type $id_admin = amministratore che effettua il cambio di password
 */
/**
...
function listaUtentiToChangePassword($pending = 1, $confirmed = 1) {
	if(connectToMySql()){
		...
		." ORDER BY b.request_date, a.cognome, a.nome " ,
		$pending, $confirmed);
 ...
 * Questa feature dovrà essere disabilitata ?!
...
function connectToMySql() {
	...
	$paramsArray = getServerConnectionParameters();
...
function createUtenteScuola($name,$surname,$user_email,$password) {
	...
	$query = sprintf("INSERT INTO scuola.utenti_scuola(cognome, nome, email, password)" .
						"VALUES ('%s','%s','%s','%s')",
	mysql_real_escape_string(strtoupper($surname), $GLOBALS['link']),
	mysql_real_escape_string(strtoupper($name), $GLOBALS['link']),
	mysql_real_escape_string($user_email, $GLOBALS['link']),
	mysql_real_escape_string(base64_encode(base64_encode($password)),$GLOBALS['link']));
...
function authenticateUser($user_email, $password, $isLogin) {
	...
	// This is the best way to perform an SQL query
	// For more examples, see mysql_real_escape_string()
	$query = sprintf("SELECT * FROM scuola.utenti_scuola WHERE email='%s' AND password='%s'",
	mysql_real_escape_string($user_email, $GLOBALS['link']),
	mysql_real_escape_string(base64_encode(base64_encode($password)),$GLOBALS['link']));
...
function changeUtenteScuolaPassword($id_utente, $user_email,$oldpassword,$newpassword) {
	if (authenticateUser($user_email, $oldpassword, FALSE)) {
		...
		"WHERE id_utente = '%s'",
		mysql_real_escape_string(base64_encode(base64_encode($newpassword)),$GLOBALS['link']),
		$id_utente);
		...
		if($result){
			...
			$msg = '<span class="error">Non è stato possibile cambiare la PASSWORD per ' .
