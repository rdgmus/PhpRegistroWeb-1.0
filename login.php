	...
	if(isset($_POST['actionLogin'])) {
		...
		if ($_POST['actionLogin'] == 'Login') {//Login button was pressed
			...
			if(empty($password)){
				...
				array("options"=>array("regexp"=>"((?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{4,20})") ))){
					...
					if(empty($password_one)){
						...
						if( $password == $password_one){
							...
							if (connectToMySql()) {
								...
								if($esito = authenticateUser($user_email, $password, TRUE)){
									registerLogEvent('LOGIN_SUCCESS', 'LOGIN IN REGISTRO SCOLASTICO:SUCCESS', 
									getUserId($user_email, $password),$_SERVER['REMOTE_ADDR']);
									...
									registerLogEventFailure("LOGIN_FAILURE","TENTATIVO DI ACCESSO CON CREDENZIALI ERRATE email=\"".
			...
			$esito = postChangePasswordRequest($cognome, $nome, $email, $hash);


			if($esito){
				//echo "esito dentro";
				$id_request = retrieveIdRequest($hash);
