...
<?php setTitle(userIsAdministrator($_COOKIE['id_utente']) ? 'Menù Amministratore' : 'Menù Utente');
 ...
 if (isset($_COOKIE['firstLogin'])){
	if ($_COOKIE['firstLogin'] == 'true'){
		...
		if (isset($_COOKIE['id_utente'])) {
			if (userIsAdministrator($_COOKIE['id_utente']) ) {
				$typeOfUser = " Administrator: ";
			}else{
				$typeOfUser = " User: ";
			}
			setMsgPopContent("<img  src='images/accept_icon.png'  width='32' height='32'>",
			"success",
			...
			?>	benvenutoUserAjax(); <?php 
			//registerLogEvent('LOGIN_SUCCESS', 'LOGIN IN REGISTRO SCOLASTICO:SUCCESS', $_COOKIE['id_utente'],$_SERVER['REMOTE_ADDR']);
		}
	}
 }
?>
	...
	<form method="post" id="userMenuForm"
		...
		<table id="userMenuTable">
			<tr>
				...
				<?php echo userIsAdministrator($_COOKIE['id_utente']) ?  '<th>Gestisci Ruoli Utenti</th>' : '' ?>
			...
			<tr>
				...
				<?php if(userIsAdministrator($_COOKIE['id_utente'])){
